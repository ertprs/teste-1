...
export class ServAtendimentoDepartamentoService {
  ...
  tipoGetAll(departamentoUid:string)
  {
    this.idCliente = this.global.dadosLogado.idCliente;
    return this.DB.collection(this.idCliente).doc('dados').collection('configuracao').doc('atend').collection('casosTipo', ref => ref.where('departamentoUid','==',departamentoUid)).snapshotChanges().pipe(
      map(action => action.map(a=>{
        const data = a.payload.doc.data() ;
        const id = a.payload.doc.id;
