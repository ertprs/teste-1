...
export class ServAtendimentoService {
  ...
  {
    this.idCliente = this.global.dadosLogado.idCliente;
    return this.DB.collection(this.idCliente).doc('dados').collection('ticket').valueChanges()
  }
  casoGetDashNovos()
  {
    this.idCliente = this.global.dadosLogado.idCliente;
    return this.DB.collection(this.idCliente).doc('dados').collection('ticket', ref=> ref.where('novo','==',1)).get()
  }
  casoGetDashSemAtendimento()
  {
    this.idCliente = this.global.dadosLogado.idCliente;
    return this.DB.collection(this.idCliente).doc('dados').collection('ticket', ref=> ref.where('qtdA','>',0)).get()
  }
  casoGetDashEmAtendimento()
  {
    this.idCliente = this.global.dadosLogado.idCliente;
    return this.DB.collection(this.idCliente).doc('dados').collection('ticket', ref=> ref.where('qtdA','==',0).where('novo','==',0).where('situacao','==',1)).get()
  }
}
