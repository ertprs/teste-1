...
import { DashviewPage } from 'src/app/modals/casos/dashview/dashview.page';
import { ModallRelAtendimentoPage } from 'src/app/modals/atendimento/relatorios/modall-rel-atendimento/modall-rel-atendimento.page';
...
export class CompAtendimentoHomeComponent implements OnInit {
  ...
  public itemsSubscribe:Subscription
  ...
  async conversasListar(){
    ...
    this.itemsSubscribe = await (await this.servConversas.atendimentoListarFiltro(this.queryStr)).subscribe((dadosReturn:any)=>{
      this.items = []
      this.dashAtResposta = 0
      dadosReturn.forEach(dados => {
        
        let tagSemResposta = 0;
        if(dados.ultMensagemData > 0 )
        {
          if(dados.tagSemRespostaTime > dados.tempoAdmitido )
          {
  ...
  async abrirRelatorios(tipo:string) {
  ...
  async abrirDashView(tipo:string) {
    const modal = await this.ctrlModal.create({
    ...
    modal.onDidDismiss().then((dados) => {
      ...
      if(dados.data !== undefined)
      {
        if(dados.data.acao === 'open')
        {
          let casoUid = dados.data.id
          this.functionExecute('compticketadd',{casoUid})
        }
      }
  ...
  ngAfterViewInit(){
    ...
    try
    {
      ...
      //contador de casos
      console.log('DASH DE CASOS')
      this.servAtendimentoService.casoGetDash().subscribe(elem=>{
        let novo = 0
        let semresposta = 0;
        let ematendimento = 0;
        elem.forEach(dados=>{
          if(dados.novo == 1){novo++}
          if(dados.qtdA > 0) { semresposta++}
          if(dados.situacao == 1 && dados.qtdA == 0 && dados.novo == 0){ematendimento++}

        })

        this.dashCasos.novos        = novo
        this.dashCasos.semresposta  = semresposta
        this.dashCasos.emandamento  = ematendimento
      })
