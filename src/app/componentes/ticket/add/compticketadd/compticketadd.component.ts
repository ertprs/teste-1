...
import { ModalcontatoconsultaPage } from 'src/app/modals/chat/contatos/modalcontatoconsulta/modalcontatoconsulta.page';
...
export class CompticketaddComponent implements OnInit {
  ...
  async abrirConsultaContato()
  {
    ...
    modal.onDidDismiss().then((dados) => {
      ...
      if(dados !== undefined)
      {
        this.dadosCaso.contatoUid         = dados.data.contatoUid
        this.dadosCaso.contatoNome        = dados.data.contatoNome
        this.dadosCaso.emailDestinatario  = dados.data.contatoEmail
        this.dadosCaso.parceiroNome       = dados.data.parceiroNome
        this.dadosCaso.parceiroUid        = dados.data.parceiroUid
        this.dadosCaso.detalhamento       = dados.data.detalhamento
  ...
  async definirSpam()
  {
    this.design.presentAlertConfirm(
      'Confirmação',
      "Confirma adicionar este remetente a lista de SPAM? Não serão mais processados e-mail`s vindos dele",
      'Sim',
      'Não'
    )
    .then(resConfirm=>{
      ...
      {
        const dadosSpam = {
          contatoEmail : this.dadosCaso.contatoEmail
        }


        this.design.presentLoading('Adicionando a lista de SPAM')
        .then(resLoading=>{
          resLoading.present()

          this.srvContatos.contatoSpamAdd(dadosSpam).subscribe(dados=>{
           
            if(dados.situacao == 'suc')
            {
              this.srvAtendimento.casoDelete(this.data.casoUid)
              .then(()=>{
                this.design.presentToast(
                  'Adicionado como SPAM',
                  'success',
                  3000
                  )
              ...
              .catch(err=>{
                let msg = 'Falha ao processa exclusao de caso | '+err
                console.log(msg)
                this.design.presentToast(
                  'Falha ao processar exclusão de caso',
                  'danger',
                  0,
                  true
                )
