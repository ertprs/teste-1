...
const Chat = {
  ...
  async transmissaoIniciar(dados:any):Promise<any>
  {
    return new Promise((resolve,reject)=>{
      CaixaEntrada.transmissaoRecuperarContatos(dados.empresaUid,dados.transmissaoUid)
      .then(resListaContatos=>{
        if(resListaContatos.situacao == 'suc')
        {
          const contatosRecuperados = resListaContatos.data.data[0]
          if(contatosRecuperados.length > 0){

            //checagem de envio
            for (const doc of contatosRecuperados) {

              //VERIFICAR SE EXISTE CONVERSA ABERTA
              CaixaEntrada.ConversaVerificar(dados.empresaUid,doc.contatoUid)
              .then(resConversaVerifica=>{
                if(resConversaVerifica.data.data[0].length > 0)
                {
                  //EXISTE CONVERSA ABERTA ADICIONAR PARA ENVIO FUTURO
                  ///QmPJcDIMLJBshGe9LDv2/chat/lista_transmissao_detalhe/{listaUid}/agendado/{contatoUid}
                  const dadosContatosAcoes = {
                    createAt: new Date().getTime(),
                    contatoUid:doc.contatoUid,
                    contatoNome:doc.contatoNome,
                    contatoWPP:doc.contatoWPP
                  }
                  db.collection(dados.empresaUid).doc('chat').collection('lista_transmissao_detalhe').doc(dados.listaUid).collection('agendado').doc(doc.contatoUid).set(dadosContatosAcoes)
                  .catch(errAcoes=>{
                    reject({situacao:'err',code:0,abortarprocesso:true,msg:'(3) | FAlha ao incluid agendamento no loop  '+doc.contatoUid})
                  })
                }
                else
                {
                  ...
                  //VERIFICAR SE CONTATO É VALIDO
                  const contatoValidoWPP = true
                  ...
                  {
                    //QmPJcDIMLJBshGe9LDv2/chat/lista_transmissao_detalhe/{listaUid}/err/{contatoUid}
                    const dadosContatosAcoes = {
                      createAt: new Date().getTime(),
                      contatoUid:doc.contatoUid,
                      contatoNome:doc.contatoNome,
                    ...
                    .catch(errAcoes=>{
                      ...
                      reject({situacao:'err',code:0,abortarprocesso:false,msg:'(3) | FAlha ao incluid agendamento no loop  '+doc.contatoUid})
                    })
                  }
              ...
              .catch(errConversaVerifica=>{
                reject({situacao:'err',code:0,abortarprocesso:true,msg:'(3) | Falha ao verificar conversa no loop  '+doc.contatoUid})
              })
          ...
          else
          {
            ...
            reject({situacao:'err',code:0,abortarprocesso:true,msg:'(2) | Nao existem contatos para enviar '})
          }
        }
        else
        {
          reject({situacao:'err',code:0,abortarprocesso:true,msg:'(1) | '+resListaContatos.msg})
       
        }
      })
      .catch(err=>{
        reject({situacao:'err',code:0,abortarprocesso:true,msg:'(0) | '+err})
  ...
  checkFileTipo(params: any) {
    ...
    try {
      ...
      const arrDocumento = ['rtf', 'dec', 'exe', 'js', 'php', 'css', 'html', 'xml', 'csv', 'pdf', 'zip', 'rar', 'txt', 'docx', 'doc', 'pptx', 'ppt', 'xlsx', 'xls','cert'];
  ...
  async conversaUpdate(params: any) {;
    try {
      ...
      //AJUSTAR STATUS DE CONVERSA NO MYSQL
      if(situacao === 2)
      { 
        CaixaEntrada.ConversasLiveChat(empresaUid,params.conversaUid)
        .then(res=>{
          console.log('Colocado em live chat com sucesso')
        })
        .catch(err=>{
          console.log('Falha ao colocar em livechat|'+err)
        })
